...
import { Mail, Phone, MapPin, Send, CheckCircle, CalendarDays, Copy, AlertTriangle } from "lucide-react";
...
const CONTACT_CSS = `
  ...
  @media (prefers-reduced-motion: reduce){
    ...
    .ct-titleAlive::after{ animation:none !important; }
  }
  ...
  @media (max-width: 560px){
    .ct-heroInner{ min-height:auto; padding-top: 86px; padding-bottom: 14px; }
  }
  ...
  .ct-rot{
    ...
    animation: ctWord 3s ease-in-out both;
  }
  @keyframes ctWord{
...
export default function Contact() {
  ...
  useEffect(() => {
    if (reduced) return;
    const id = window.setInterval(() => setRotIdx((p) => (p + 1) % rotating.length), 3000);
  ...
  useEffect(() => {
    if (isDemo && !formData.message) {
      setFormData((p) => ({ ...p, message: t("contact.form.demo_prefill") }));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isDemo]);
        ...
        <div className="ct-heroInner">
          <div className="ct-container">
            <div className="mx-auto max-w-[980px] text-center">
              ...
              <h1 className="mt-6 text-white break-words ct-reveal">
                <span className="ct-titleAlive block text-[40px] leading-[1.05] sm:text-[60px] font-semibold">
              ...
              <p className="mt-5 text-[16px] sm:text-[18px] leading-[1.7] text-white/70 break-words ct-reveal">
                <span className="ct-subline">
                  ...
                  <span className="ct-subTail">— {tail}</span>
                </span>
              </p>

              <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center ct-reveal">
                <a
                  href={WHATSAPP_LINK}
                  target="_blank"
              ...
              <div className="ct-divider ct-reveal" />
            </div>
          </div>
        </div>

...
import { Globe2, ShieldCheck, CheckCircle2, ChevronDown } from "lucide-react";
...
function ServicePage({
  ...
  const services: ServiceLink[] = useMemo(
    () => [
      { id: "chatbot-24-7", path: "/services/chatbot-24-7", title: () => "Chatbot 24/7" },
      {
        ...
        title: (l) =>
          l === "az" ? "Business Workflows" : l === "ru" ? "Бизнес-воркфлоу" : l === "tr" ? "İş Akışları" : l === "es" ? "Workflows" : "Business Workflows",
      },
      {
        id: "websites",
  ...
  const dropdownItems = useMemo(
    () =>
      services
        .filter((s) => s.path !== "/services/websites")
      ...
      <div className="container">
        ...
        <div className="svc-below">
          ...
          <div>
            ...
            <div className="svc-card" data-reveal style={{ marginTop: 14 }}>
              <div className="svc-card__title">
                <Globe2 size={18} />
                <span className="svc-grad svc-shimmer">{lang === "az" ? "Frontend & UX" : "Frontend & UX"}</span>
              </div>
              <div className="svc-card__desc">
                {lang === "az"
                  ? "Mobil-first premium dizayn, smooth animasiya və yüksək performans. SEO əsasları və konversiya yönümlü struktur."
                  : "Premium mobile-first UI, smooth motion and high performance, with conversion-first structure."}
              </div>
              <div style={{ marginTop: 12 }}>
                <Feature title={lang === "az" ? "Performance" : "Performance"} desc={lang === "az" ? "Video optimizasiya, lazy-load, caching və minimal JS." : "Optimized assets, lazy-load, caching and minimal JS."} />
                <Feature title={lang === "az" ? "SEO" : "SEO"} desc={lang === "az" ? "Meta, struktur, sitemap, clean URL və OpenGraph." : "Meta, structure, sitemap, clean URLs and OpenGraph."} />
                <Feature
                  title={lang === "az" ? "Conversion UX" : "Conversion UX"}
                  desc={lang === "az" ? "CTA, pricing flow, lead capture və A/B-ready struktur." : "CTAs, pricing flow, lead capture and A/B-ready structure."}
                />
              </div>
            </div>
          </div>
          ...
          <div className="svc-card" data-reveal style={{ marginTop: 14 }}>
            <div className="svc-card__title">
              <ShieldCheck size={18} />
              <span className="svc-grad svc-shimmer">{lang === "az" ? "Backend & Admin" : "Backend & Admin"}</span>
            </div>
            <div className="svc-card__desc">
              {lang === "az"
                ? "Blog, media library, leads, multi-lang və deploy flow (Cloudflare/Netlify + Railway) — hamısı hazır sistem."
                : "Blog, media library, leads, multi-lang and deploy flow (Cloudflare/Netlify + Railway) — ready stack."}
            </div>
            <div style={{ marginTop: 12 }}>
              <Feature title={lang === "az" ? "Deploy" : "Deploy"} desc={lang === "az" ? "CI/CD, env vars, domain, SSL və monitoring." : "CI/CD, env vars, domain, SSL and monitoring."} />
              <Feature title={lang === "az" ? "Security" : "Security"} desc={lang === "az" ? "JWT, rate limit, admin allowlist və audit." : "JWT, rate-limits, admin allowlist and audit."} />
              <Feature title={lang === "az" ? "Admin tools" : "Admin tools"} desc={lang === "az" ? "Blog, media, leads, kontent idarəsi." : "Blog, media, leads, content management."} />
            </div>
          </div>
        </div>
      </div>
